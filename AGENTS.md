...
- Every push to `main` deploys to https://bando.toom.as (S3+CloudFront, GitHub OIDC —
...
- The sync backend (`backend/handler.mjs`, `infra/backend.tf` — Cognito + HTTP API +
  Lambda + DynamoDB at api.bando.toom.as) deploys via `terraform -chdir=infra apply`,
